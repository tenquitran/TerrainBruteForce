...
bool TiledTexture::initialize(const GLuint& program, const std::string& textureSamplerName, const std::vector<glm::vec3>& vertices)
{
...
#else
	unsigned char *pTextureData = SOIL_load_image(m_fileName.c_str(), &m_textureWidth, &m_textureHeight, 0, SOIL_LOAD_RGB);
	if (!pTextureData)
	{
		std::wcerr << L"Failed to load terrain texture " << m_fileName.c_str() << ": " << SOIL_last_result() << '\n';
		return false;
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB8, m_textureWidth, m_textureHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, pTextureData);

	SOIL_free_image_data(pTextureData);
#endif
