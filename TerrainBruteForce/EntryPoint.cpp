...
int APIENTRY _tWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
	...
	try
	{
		...
		WindowInfo wndInfo(ClientWidth, ClientHeight, IDS_APP_TITLE, IDC_TERRAINBRUTEFORCE,
			IDI_TERRAINBRUTEFORCE, IDI_SMALL, IDC_TERRAINBRUTEFORCE);

		OpenGLInfo openGLInfo(OpenGLVersionMajor, OpenGLVersionMinor, FieldOfView, FrustumNear, FrustumFar);

		const GLfloat TerrainScaleFactor = 0.35f;

		MainWindow mainWindow(hInstance, wndInfo, openGLInfo, TerrainScaleFactor);
